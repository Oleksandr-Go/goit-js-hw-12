...
`).join("");y.insertAdjacentHTML("beforeend",o),P.refresh()}function $(){y.innerHTML=""}function h(){f.classList.add("is-visible")}function d(){f.classList.remove("is-visible")}function v(){m.classList.replace("load-hidden","load-more")}function c(){m.classList.replace("load-more","load-hidden")}const O=document.querySelector(".form"),L=document.querySelector('input[name="search-text"]'),u=document.querySelector(".load-more");u.addEventListener("click",E);O.addEventListener("submit",B);let i=1;const b=15;function B(r){r.preventDefault(),c(),$(),h();const o=L.value.toLowerCase().trim();i=1,p(o,i).then(s=>{const a=Math.ceil(s.totalHits/b);if(!s.hits.length){l.error({message:"Sorry, there are no images matching your search query. Please try again!",position:"center"}),d(),c();return}g(s.hits),i<a?v():(c(),l.info({position:"topRight",message:"We're sorry, but you've reached the end of search results."}))}).catch(s=>{l.error({message:"Sorry, something went wrong.",position:"center"})}).finally(()=>d())}async function E(){u.disabled=!0,i+=1;try{c(),h();const r=await p(L.value.trim(),i),o=Math.ceil(r.totalHits/b);g(r.hits);const a=document.querySelector(".gallery-item").getBoundingClientRect().height;window.scrollBy({left:0,top:a*2,behavior:"smooth"}),u.disabled=!1,i>=o?(c(),l.info({position:"topRight",message:"We're sorry, but you've reached the end of search results."})):v()}catch{l.error({message:"Sorry, something went wrong.",position:"center"})}finally{d()}}
